...
class FundaSpider(scrapy.Spider):
    ...
    def parse_details(self,response):
        sales_agents = response.css('div.object-primardy > section.object-contact > div > h3.object-contact-aanbieder-name > a::text').extract()
        ...
        yield {
            ...
            'numberrooms': int(max(re.findall("\d+", response.css('#content > div > div > div.object-primary > section:nth-child(7) > div > dl:nth-child(11) > dd:nth-child(2)::text').extract_first()))), # Robin
            'numberbathrooms': int(max(re.findall("\d+",response.css('#content > div > div > div.object-primary > section:nth-child(7) > div > dl:nth-child(11) > dd:nth-child(4)::text').extract_first()))), # Robin
            'energylabelclass': str(response.css('span.energielabel::text').extract_first()).strip().rstrip("\n").rstrip("\r"), # Robin
            'surface': int(max(re.findall("\d+",response.css('#content > div > div > div.object-primary > section:nth-child(7) > div > dl:nth-child(8) > dd:nth-child(5)::text').extract_first()))), # Emmanuel
            'buying_agent': sales_agents[1] if len(sales_agents)==2 else None, # Robin
            
            'sellingDate': datetime.strptime(str(response.css('#content > div > div > div.object-primary > section:nth-child(3) > div > dl > dd:nth-child(4)::text').extract_first()).strip().rstrip("\n").rstrip("\r"), '%B %d, %Y').date(), # Robin
...
# This class contains a spider that scrapes the housing information on not sold houses on the funda website
# © Robin Kratschmayr, Felicia Betten, Emmanuel Owusu Annim, Baris Orman
class FundaSpider(scrapy.Spider):
    name = 'funda_not_sold'
    allowed_domains = ['funda.nl']
    start_urls = ['https://www.funda.nl/koop/gemeente-amsterdam/']
